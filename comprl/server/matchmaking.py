...
# Type of a player entry in the queue, containing the player ID, user ID, mu, sigma and time they joined the queue
...
def _rate_match_quality(
    ...
    """returns the expected fairness of the game,
    considering the waiting time of both players."""
    now = datetime.now()
    combined_waiting_time = (time_stamp_p1 - now).total_seconds() + (
        time_stamp_p2 - now
    ).total_seconds()
    # calculate a bonus if the players waited long 

...
class rock_paper_scissors(IGame):
    ...
    def _update_environment(self):
        match self.current_actions[0]:
            case 0: #rock vs.
                if(self.current_actions[1]==2): #
                    self.env[0] = self.env[0]+1
                elif(self.current_actions[1]==1):
                    self.env[1] = self.env[1]+1
            case 1:
                if(self.current_actions[1]==0):
                    self.env[0] = self.env[0]+1
                elif(self.current_actions[1]==2):
                    self.env[1] = self.env[1]+1
            case 2:
                if(self.current_actions[1]==1):
                    self.env[0] = self.env[0]+1
                elif(self.current_actions[1]==0):
                    self.env[1] = self.env[1]+1
